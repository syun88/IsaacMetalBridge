...
if ! grep -F 'VULKAN_ICD discovered="IsaacMetalBridge (' "${guest_log}" >/dev/null \
    ...
    || ! grep -F 'VULKAN_SPARSE_IMAGE format=RGBA8_SRGB tile=128x128 map=passed unmap=passed backend=Metal' "${guest_log}" >/dev/null \
    || ! grep -F 'VULKAN_RASTER triangle=64x64 format=RGBA8' "${guest_log}" >/dev/null; then
    echo "test-isaac-vulkan-injection: injected ICD did not complete Metal compute, texel-buffer, RGBA8_SNORM, RGBA8 sRGB sparse residency, and raster" >&2
    sed -n '1,240p' "${adapter_log}" >&2
    sed -n '1,240p' "${guest_log}" >&2
    exit 1
fi
...
echo "test-isaac-vulkan-injection: real Isaac Sim ARM64 image completed Metal compute, texel-buffer, RGBA8_SNORM, RGBA8 sRGB sparse residency, and raster"
