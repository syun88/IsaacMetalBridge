...
public struct RaySphereLight: Sendable, Equatable {
    ...
    /// Normalized world-space primary emission direction from UsdLuxShapingAPI.
    public let shapingAxis: SIMD3<Float>
    public let shapingConeAngleDegrees: Float
    public let shapingConeSoftness: Float
    ...
    public init(
        ...
        halfExtentU: Float = 0,
        halfExtentV: Float = 0,
        shapingAxis: SIMD3<Float> = .zero,
        shapingConeAngleDegrees: Float = 0,
        ...
        self.shapingAxis = shapingAxis
        self.shapingConeAngleDegrees = shapingConeAngleDegrees
        self.shapingConeSoftness = shapingConeSoftness
...
public final class MetalGPUBackend: BridgeGPUBackend, @unchecked Sendable {
        ...
        if (hasGridHit) {
            ...
            if ((cameraOptions & (2u | 8u | 16u)) != 0
                ...
                for (uint sphereLightIndex = 0u;
                    ...
                    const bool isPlanarAreaLight =
                        activeShape == 2u || activeShape == 3u;
                    const bool isCylinderLight = activeShape == 6u;
                    const bool cylinderHasRadius = isCylinderLight
                        && activeAxisVAndHalfExtent.w > 0.000001
                        && activeColorAndRadius.w > 0.000001;
                    const bool isAreaLight =
                        isPlanarAreaLight || isCylinderLight;
                    const uint areaSampleCount = isCylinderLight
                        ? (cylinderHasRadius ? 8u : 2u)
                        : (isPlanarAreaLight ? 4u : 1u);
                    ...
                    if (isPlanarAreaLight) {
                        ...
                        const float3 axisU = normalize(activeAxisUAndHalfExtent.xyz);
                        const float3 axisV = normalize(activeAxisVAndHalfExtent.xyz);
                        const float3 axisW = normalize(cross(axisU, axisV));
                        const float axialSign =
                            (areaSampleIndex & 1u) == 0u ? -1.0 : 1.0;
                        sampledLightPosition += axisU
                            * activeAxisUAndHalfExtent.w * axialSign * 0.65;
                        if (cylinderHasRadius) {
                            const uint radialIndex = areaSampleIndex >> 1u;
                            const float radialAngle =
                                (float(radialIndex) + 0.5) * 1.5707963267948966;
                            const float radialCosine = cos(radialAngle);
                            const float radialSine = sin(radialAngle);
                            const float radiusV = max(
                                activeAxisVAndHalfExtent.w, 0.000001
                            );
                            const float radiusW = max(
                                activeColorAndRadius.w, 0.000001
                            );
                            sampledLightPosition +=
                                axisV * activeAxisVAndHalfExtent.w * radialCosine
                                + axisW * activeColorAndRadius.w * radialSine;
                            // Gradient of the elliptical cross-section gives
                            // the correct outward surface normal.
                            emitterNormal = normalize(
                                axisV * (radialCosine / radiusV)
                                    + axisW * (radialSine / radiusW)
                            );
                        }
                    }
                    ...
                    if (hasShaping) {
                        ...
                        const float axisDot = clamp(
                            dot(shapingAxis, -lightDirection), -1.0, 1.0
                        );
                        const float offAxisAngle = acos(axisDot);
                        const float coneCutoff = clamp(
                            activeShapingAxisAndCone.w, 0.0, 180.0
                        ) * 0.0174532925199433;
                        const float coneSoftness = clamp(
                            activeShapingSoftnessFocusTintRG.x, 0.0, 1.0
                        );
                        if (coneSoftness <= 0.000001 || coneCutoff <= 0.000001) {
                            shapingScalar = offAxisAngle <= coneCutoff ? 1.0 : 0.0;
                        } else {
                            const float smoothStart = coneCutoff
                                * (1.0 - coneSoftness);
                            shapingScalar = 1.0 - smoothstep(
                                smoothStart, coneCutoff, offAxisAngle
                            );
                        }
                        const float focus = max(
                            activeShapingSoftnessFocusTintRG.y, 0.0
                        );
                        const float focusFactor = focus <= 0.000001
                            ? 1.0
                            : pow(abs(axisDot), focus);
                        const float3 focusTint = max(
                            float3(
                                activeShapingSoftnessFocusTintRG.z,
                                activeShapingSoftnessFocusTintRG.w,
                                activeShapingTintBAndFlags.x
                            ),
                            float3(0.0)
                        );
                    ...
                    // from the receiver to the sampled emitter position.
                    const float emitterFacing = (isPlanarAreaLight
                    ...
                    const float radius = isCylinderLight
                        ? max(
                            max(
                                activeColorAndRadius.w,
                                activeAxisVAndHalfExtent.w
                            ),
                            max(activeAxisUAndHalfExtent.w * 0.25, 1.0)
                        )
                        : max(activeColorAndRadius.w, 1.0);
                    ...
                    const float diffuseResponse = isAreaLight
                        ? diffuse * visibility
                        : shapingScalar * (0.25 + 0.75 * diffuse * visibility);
    ...
    public static func makeDefault() -> MetalGPUBackend? {
        ...
        do {
            ...
            let message = "imb-host: Metal backend initialization failed: \(error)\n"
        ...
        let additionalSphereLightRecords = additionalSphereLights.flatMap {
            [
                ...
                SIMD4<Float>(
                    ...
                    $0.shapingAxis.y,
                    $0.shapingAxis.z,
                    $0.shapingConeAngleDegrees
                ),
                SIMD4<Float>(
                    $0.shapingConeSoftness,
                    $0.shapingFocus,
                    $0.shapingFocusTint.x,
                    $0.shapingFocusTint.y
                ),
                SIMD4<Float>(
                    $0.shapingFocusTint.z,
                    $0.hasShaping ? 1 : 0,
                    0,
                    0
                ),
