...
The 2026-08-11 Full validation used protocol 1.18 and scene-state v16. It reported the host as Apple M4, exposed 24,142 MB to Kit, loaded bundled ROS 2 Jazzy, enabled all 204 captured compute pipelines, published a complete six-light list containing RectLight, DiskLight, two DistantLights, and two DomeLights, and exited zero. Its checked 1440x900 frame retained the Full workspace, correct sRGB texture color, transparent geometry, and shadows. The prior sparse `VK_FORMAT_R8G8B8A8_SRGB` format-43 rejection and targeted Render Graph/multiTick/Sensor/Hydra errors were absent. NGX and OptiX initialization remain NVIDIA-only and are not counted as implemented Metal features.
