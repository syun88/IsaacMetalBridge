...
| `CREATE_COMPUTE_PIPELINE` | Control/data | Validate SPIR-V bytes and an entry-point name, translate to MSL, compile a real Metal compute pipeline, and return its session-scoped resource ID. The optional four-byte reply payload is a little-endian creation-flags word; bit 0 (`SOFTWARE_FP64_EXECUTION_REQUIRED`) means the selected entry point contains software-lowered binary64 operations rather than declaration-only matrix members. An empty reply remains accepted conservatively for compatibility. |
