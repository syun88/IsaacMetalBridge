...
The magic value is `0x31424d49`, which appears as `IMB1` on the wire. Version 1.22 is the only implemented protocol version. It retains Metal sparse-image tile mappings, generic compute descriptors, inline independent Vulkan samplers, formats 8/9/10/11, tightly packed ordinary 3D images, the renderer-owned empty-stage guide, and bounded camera/light ray submissions from version 1.21. Scene-state v20 is the current per-run Mesh/material/light sideband; v3-v19 remain readable by the ICD.
...
Bytes 80-119 carry a normalized world emission axis, cone cutoff angle in degrees, cone softness, focus exponent, focus tint, and a 32-bit shaping flag. Bit zero means `UsdLuxShapingAPI` was explicitly applied; all shaping values must be zero otherwise. Bytes 120-151 carry a first color-image resource ID, an IES-profile resource ID, finite `ies:angleScale`/multiplier values, a texture flag word, and reserved zero. Flag bit 0 enables RectLight sRGB emission, bit 1 enables a linear angular IES LUT, and bit 2 records authored `ies:normalize`. Protocol 1.22 adds bit 3 for a DomeLight 2:1 latlong environment and bit 4 for Radiance RGBE rather than sRGB. A textured Dome uses values 4-12 for normalized world-space local +X/+Y/+Z axes; values 13-15 stay zero. The first image is mutually exclusive between Rect and Dome use. Scene-state v20 keeps the 160-byte header, complete list, and bounded light-image table; v3-v19 remain readable.
...
Version 1.22 is the current bounded data plane. In addition to the v1.21 paths described below, it validates and binds transformed DomeLight latlong sRGB/RGBE images and uses them for Metal miss/background, diffuse environment, and specular environment sampling. Scene-state v20 publishes those axes, flags, and image bytes; v3-v19 remain accepted for backward compatibility. OpenEXR and non-latlong Dome formats are not decoded.
