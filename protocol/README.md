...
The magic value is `0x31424d49`, which appears as `IMB1` on the wire. Version 1.18 is the only implemented version. It retains Metal sparse-image tile mappings and generic compute descriptor records, adds inline independent Vulkan sampler state without changing the fixed 48-byte binding record, adds image format 8 for four-channel unsigned-byte storage (`VK_FORMAT_R8G8B8A8_UINT` / `MTLPixelFormat.rgba8Uint`), format 9 for four-channel signed-normalized storage (`VK_FORMAT_R8G8B8A8_SNORM` / `MTLPixelFormat.rgba8Snorm`), plus sparse-capable format 10 for sRGB color storage (`VK_FORMAT_R8G8B8A8_SRGB` / `MTLPixelFormat.rgba8Unorm_srgb`), while ordinary image option bit 1 plus option bits 16-31 carry a tightly packed 3D image and its depth, leaving option zero as 2D and option bit 0 as sparse. It also carries optional live Kit camera plus bounded positional-, distant-, and dome-light records in bounded ray submissions. Positional records preserve SphereLight directly and carry RectLight/DiskLight world axes and half extents. Version 1.18 assigns `TRACE_RAYS` option bit 4 to the renderer-owned empty-stage guide: its acceleration-structure resource ID is zero, light bits are forbidden, and the live camera remains optional. The same command can be submitted more than once for viewport and Camera Render Product traces; scene-state v16 Mesh geometry, authored-normal, corner-UV, bounded RGBA8 base/parameter/normal-image, roughness, metallic, emission, standard opacity, and opacityThreshold data plus sensor files remain per-run sidebands.

Version 1.18 assigns `TRACE_RAYS` option bit 5 to a complete light-list tail. The unchanged 168-byte base command is followed by an 8-byte count/reserved header and at most sixteen 80-byte records. Each record preserves its positional (`SphereLight`, `RectLight`, or `DiskLight`), distant, or dome schema plus a stable USD path hash; RectLight/DiskLight use the additional float fields for two transformed in-plane axes and their half extents. Scene-state v16 has a 160-byte fixed header followed by the same complete list and admits up to eight positional, four distant, and four dome lights total. Scene-state v3-v15 remains readable.
