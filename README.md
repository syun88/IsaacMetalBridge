...
- With `--window` and no explicit scene flag, Kit retains its normal `World`/`Environment` empty stage and no diagnostic USD is opened. Protocol 1.17 renders the familiar perspective grid and X/Y axes directly into the landscape viewport presentation textures, follows the published `/OmniverseKit_Persp` camera, and excludes Kit's separate font/icon atlas. A checked Full 1440×900 run loaded ROS 2 Jazzy, created and enabled all 204 Metal compute pipelines, reported zero Render Graph/multiTick/Sensor/Hydra/memory-map failures, and preserved readable Stage, Property, and Content UI.
...
- The 2026-08-11 protocol-1.17 Full validation used the real Isaac Sim 6.0.1 ARM64 image with bundled ROS 2 Jazzy, reported Apple M4 and 24,142 MB to Kit, execution-enabled all 204 captured compute pipelines, transported scene-state v15 with two standard-opacity Meshes and three additional lights, and exited zero. The host reported `lights=2/2/2`; the checked 1440x900 frame retains the Full workspace and textured sRGB cube while the red opacity test canopy is correctly absent. The former `VK_FORMAT_R8G8B8A8_SRGB` format-43 sparse-image rejection and all targeted Render Graph/multiTick/Sensor/Hydra errors were absent. NGX and OptiX remain NVIDIA-only and log three expected initialization errors.
...
- Atomic scene-state v15 transport of visible USD Mesh points/indices/authored corner normals/corner UVs, disjoint material-bound GeomSubset faces, one deduplicated bounded RGBA8 texture table, and direct roughness/metallic/emission constants; deterministic per-Mesh Metal BLAS creation with derived tangent/bitangent data and authored world-transform TLAS instances; v3-v14 parsing and v3/v4 bounds matching remain available
