...
- Protocol 1.22 / scene-state v20 extends that verified path with transformed 2:1 DomeLight latlong environments. Bounded PNG/JPEG images are decoded as sRGB; Radiance `.hdr`/`.pic` images retain linear high range through RGBE8 transport. Metal samples the environment for visible misses, normal-directed diffuse response, and reflected-view specular response using OpenUSD's +Y pole, +Z zero-longitude, and transformed local axes. OpenEXR and the non-latlong Dome formats remain outside this bounded decoder.
...
- The subsequent protocol-1.22 / scene-state-v20 Full validation published `/World/FillLight` with its 8×4 latlong image as light texture index 2, transported the complete eight-light list, and reported `lights=4/2/2`. Full 6.0.1 and internal ROS 2 Jazzy reached `app ready`; the checked 1440×900 frame visibly contains the transformed environment background and lit USD scene inside the complete Stage/Property/Content workspace. The container exited zero and `container list` was empty afterward. Metal regressions separately verify pole orientation, prim-transform rotation, background sampling, and RGBE radiance above one.
...
8. Completed Dome environment slice: protocol 1.22 and scene-state v20 add bounded transformed 2:1 latlong sRGB/Radiance-RGBE images, visible miss/background sampling, and diffuse/specular environment response. Remaining work includes OpenEXR and other Dome projections, broader material networks, colored transmission, Geometry/Portal lights, broader IES variants, nested instancing, robust deformation, and native RTX sensor arrays.
...
- Transformed DomeLight 2:1 latlong backgrounds and diffuse/specular environment sampling, including sRGB images and Radiance RGBE high-range transport
